...
class FornecedorModel(Base):
	...
	def __init__(self, empresa, senha, email, nome, endereco, cnpj, tipo, ativo, n_funcionarios):
		self.nome = nome
	...
	def modificar(cls, _id, data):
		...
		newItem.n_funcionarios = data['n_funcionarios']
		db.session.commit()	
	...
	@classmethod
	def encontrar_pelo_nome(cls, empresa):
		return cls.query.filter_by(empresa=empresa).first()
